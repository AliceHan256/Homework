...
7. Next, open the terminal (Mac) or Git Bash (Windows).
    ...
    * Window users need to set path for folder containing anaconda3 and anaconda3/scripts.
    You can do it easily,  hit the windows key → enter environment → choose from settings → edit environment variables for your account → select Path   variable → Edit → New.
    e.g. for PK below are the 2 the path  variables, it would be different for your machine. Please check and then add properly.
    
set PATH=%PATH%;C:\Users\anaconda3;C:\Users\\anaconda3\Scripts\ 
hen restart git bash and try  below command,  you should receive an output
